...
- Mantener registro histórico de costos en `docs/reports/`
...
```bash
grep -n -A2 "finanzas" specs/NNN-feature/tasks.md
```
...
git checkout -b <feature-id>/finanzas
```

```text
[Finanzas Agent] Comenzando analisis de costos...
```

### 3. Reportar Progreso

```text
[Finanzas Agent] Reporte parcial generado
[Finanzas Agent] @knowledge-x6e Gasto semanal: $XX.XX
```

Si detectás un gasto anómalo, agregá un checkbox en la sección del Planner:

```markdown
- [ ] Alerta: Backend Agent excede presupuesto semanal → Planner Agent (P0)
```

### 4. Completar Tu Trabajo

```text
[Finanzas Agent] Completado:
- Reporte semanal generado en docs/reports/
...
# 3. Escribir el reporte en docs/reports/YYYY-MM-DD-weekly-cost-report.md
```

```text
[Finanzas Agent] Reporte Semanal (2026-02-17 al 2026-02-23):
...
git commit -m "docs(reports): weekly cost report 2026-02-23"
git push
```
...
```markdown
- [ ] Alerta: Gasto diario excede umbral ($XX vs $YY promedio) → Planner Agent (P0)
  Agente: Backend (knowledge-vlf). Gasto hoy: $XX.XX (3x el promedio).
...
```markdown
- [ ] Optimizacion: Evaluar downgrade de modelo para DevOps → Planner Agent (P2)
```

```text
[Finanzas Agent] Analisis:
...
- Riesgo: Posible reduccion en calidad de IaC compleja
```
...
5. **Costo por Iniciativa**: Correlacionar gasto con `specs/NNN-feature/` completadas
6. **Creditos Restantes**: Alerta cuando quede <20%
7. **Eficiencia**: Tokens por checkbox de `tasks.md` completado
...
```text
[Finanzas Agent] @knowledge-x6e Monthly cost report:
- Total: $XXX.XX
- Budget remaining: XX%
...
```markdown
- [ ] Automatizar reporte semanal de costos OpenRouter → DevOps Agent (P2)
  Necesito: cron job semanal que consulte OpenRouter API, parsee la respuesta,
  genere el reporte y lo guarde en docs/reports/. Alertar via checkbox en tasks.md
  si excede umbral.
```

### Con Todos los Agentes
```text
[Finanzas Agent] Recordatorio para todos:
...
1. **Completar reportes en progreso**
   ```bash
   git add docs/reports/
   git commit -m "docs(reports): weekly cost report"
   git push
   ```

2. **Actualizar tasks.md**
   ```markdown
   - [x] T050 Reporte semanal de costos
   - [ ] T051 Automatizacion de alertas — continuar proxima sesion
   ```

3. **Reportar estado**
   ```text
   [Finanzas Agent] Session end:
   - Completado: X tareas
   - En progreso: X
...
4. **Commit y push**
   ```bash
   git add . specs/
   git commit -m "docs(reports): [resumen]"
   git push
...
4. **Contexto**: Correlaciona costos con iniciativas completadas (costo =/= desperdicio)
5. **Historico**: Mantiene registro en `docs/reports/` para identificar tendencias
...
```text
[Finanzas Agent] Reporte Inicial de Costos OpenRouter:
...
# 2. Guardar y push
git add docs/reports/
git commit -m "docs(reports): initial cost report and baseline"
git push
```
...
gh pr create \
  ...
  --body "Closes finanzas section of specs/<feature-id>/tasks.md"
```
