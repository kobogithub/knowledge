...
- [x] T006 ✅ **Passed 2026-08-17.** Ran a throwaway push-triggered workflow (`workflow_dispatch` needs the file on the default branch, which this deliberately never reached). Read the tap's formula (`v0.10.0`, blob `5e649b40`), created `.tap-token-probe`, deleted it, and confirmed the tap was left clean. Workflow and branch removed after. Original: Verify `TAP_GITHUB_TOKEN` can read and write `$TAP` by making a trivial round-trip from a scratch workflow run or a local `curl` against the GitHub API using the token: read `Formula/kn.rb`, write an unrelated file, delete it. Confirms permission scope and that the token is not expired (contract C2.6)
...
- [x] T018 [US2] ✅ Added with `needs: [create-release]`. Original: Add a `publish-formula` job to `.github/workflows/release.yml`, with `needs: [create-release]` so it runs only after the release is successfully published (C2.3, FR-011)
- [x] T019 [US2] ✅ Renders via a Python step that substitutes both URLs and both checksums, and **aborts if the substitution counts are not exactly (1,1,2)** — a silent no-match would otherwise publish a formula pointing at the old release. Logic verified locally against the real formula. Original: In `publish-formula`, render `Formula/kn.rb` from the repo by substituting `version`, both `url`s and both `sha256`s, then commit and push the result to `$TAP` at `Formula/kn.rb`. The authored formula stays the reviewable source of truth; the tap becomes a generated output (research R2, C1.6, C1.7)
- [x] T020 [US2] ✅ Both computed from the artifacts downloaded back from the published release, with `curl -fsSL` so a missing asset fails the job. Original: Compute both checksums inside the job **from the artifacts actually uploaded to the release**, never from a value stored in the repo. A hand-maintained checksum is the drift mechanism this feature exists to remove (C2.5, FR-012)
- [x] T021 [US2] ✅ Uses `secrets.TAP_GITHUB_TOKEN` via `x-access-token` in the clone URL, and fails fast if the secret is empty. Original: Authenticate the push with `secrets.TAP_GITHUB_TOKEN`. `GITHUB_TOKEN` is scoped to the running repository and **cannot** push to `$TAP` (C2.6, research R3)
- [x] T022 [US2] ✅ `git diff --quiet` before committing — if the rendered formula already matches the tap, the job exits 0 without an empty commit. Original: Ensure the job is a no-op-safe re-run: publishing the same tag twice must leave `$TAP` in a consistent state rather than a half-applied edit (C2.7, spec edge case 5)
- [x] T023 [US2] ⚠️ **Verified structurally, not empirically.** `needs: [create-release]` means GitHub skips `publish-formula` unless every needed job succeeded, so a failed build cannot reach the tap. The second guard is `curl -fsSL` on the published assets: a missing artifact fails the step before anything is rendered or pushed. **No failed release was deliberately staged** — doing so would mean publishing a broken tag to a public repo. Recorded as unverified-by-execution rather than checked off as tested. Original: Verify the failure path: if `build-binaries` or `create-release` fails, `publish-formula` must not run and `$TAP` must be untouched. A formula pointing at a nonexistent download is worse than a stale one (C2.4, FR-011, spec edge case 2)
- [x] T024 [P] [US2] ✅ Created. Weekly cron plus `workflow_dispatch`. Parses the version from the formula's **URL**, not from a `version` line — `brew audit --strict` rejects an explicit `version` as redundant, so that line no longer exists. Original: Create `.github/workflows/tap-drift-check.yml`: on a schedule, read the `version` declared in `$TAP`'s `Formula/kn.rb`, read the newest release tag from this repo, and fail when they disagree (C2.9, C2.10, FR-013)
- [x] T025 [US2] ✅ **Decided against relying on the email default.** A failing scheduled workflow only notifies if the owner has Actions email notifications enabled — an account setting the workflow cannot see or guarantee, so assuming it would leave FR-013 resting on an unverified premise. The check now **files a GitHub issue** labelled `tap-drift`, reusing one open issue instead of filing weekly duplicates. Self-contained, visible in the repository regardless of notification settings. Original: Decide and implement how a drift-check failure actually reaches the maintainer, and record the decision in the workflow file. GitHub emails the repository owner on scheduled-workflow failure by default, which may well be sufficient for a solo maintainer — but confirm that default is active for this account rather than assuming it. A check that fails silently in the Actions tab leaves I1 as unobservable as it is today (FR-013)
- [x] T026 [US2] ✅ Separate workflow file, own `schedule:` trigger, no `needs` on anything in `release.yml`. Original: Confirm the drift check runs **independently of the release workflow**, on its own schedule. An assertion inside the release job cannot detect a push that later reverted or a credential that expired months after the last release — exactly the class of failure that produced the current 0.9.0/0.10.0 gap (C2.11, research R6)
- [ ] T027 [US2] Cut a release and confirm the tap updates end-to-end with zero manual steps (SC-003). Then run quickstart **V6** (failed build leaves tap untouched), **V7** (published checksums match published bytes) and **V8** (drift check fails on a deliberately stale formula, passes on the current one). This is the first exercise of the full pipeline — every earlier verification reused an existing release
  - **V7 adelantado y en verde (2026-09-12, contra `v0.12.0`)**: el `checksums.txt` publicado declara `a520e094bbe9e43862ba06b06af0ccb45ef12e215b7c73c16f9d879eaea161e9` y el sha256 del tarball publicado es ese mismo. Coinciden.
  - Nota de V7: `checksums.txt` guarda la ruta del build (`./kn-macos-arm64/kn-macos-arm64.tar.gz`), no el nombre del asset, así que un `shasum -a 256 -c checksums.txt` a secas sobre los assets descargados falla con "No such file or directory" aunque el hash esté bien. `install.sh` no se ve afectado: matchea por basename a propósito (`verify_checksum`, comentario en las líneas 395-401). Queda como detalle de cara al usuario, no como defecto de la verificación.
  - **Sigue bloqueada la parte central**: SC-003, V6 y V8 exigen *cortar un release nuevo*. No se puede cerrar reusando `v0.12.0`, que es justamente lo que la tarea prohíbe.
...
- [x] T028 [US3] ✅ Matrix removed entirely — single `runs-on: macos-latest` job building `aarch64-apple-darwin` natively. Original: Reduce the `build-binaries` matrix in `.github/workflows/release.yml` (lines ~27-45) to the single existing `aarch64-apple-darwin` / `macos-latest` entry. Remove the `x86_64-unknown-linux-gnu` and `x86_64-apple-darwin` entries (FR-002, C2.1)
- [x] T029 [US3] ✅ **Confirmed applied, not forgotten.** Asset name is still `kn-macos-arm64.tar.gz`; a comment in the workflow records why. Original: **Do not rename the artifact.** It must stay `kn-macos-arm64.tar.gz`. `install.sh` selects on this name and in-tool `kn update` parses it — `RELEASE.md:273` records that the `kn-{os}-{arch}.tar.gz` shape is required for update to work. Renaming it now that there is only one target would break self-update for every already-installed copy (C2.2, research R5). This task is a deliberate no-op — check it off to confirm the decision was applied, not forgotten
- [x] T030 [US3] ✅ Rewritten: Homebrew first, one platform listed. **Also fixed a pre-existing bug** — the template used a quoted heredoc, so `$(cat checksums.txt)` was never expanded and every release through v0.10.0 published that string literally. Checksums are now appended after the heredoc. Original: Update the release-notes template in `.github/workflows/release.yml` (lines ~109-111) to list only the Apple Silicon download. It currently advertises Linux x86_64 and macOS x86_64 downloads that will no longer exist (C2.8, FR-016)
- [x] T031 [US3] Run quickstart **V5** against a real release: `gh release view <tag> --json assets` returns exactly `kn-macos-arm64.tar.gz` and `checksums.txt` (SC-004, C2.1)
  - **Verificado 2026-09-12 contra `v0.12.0`**: los assets publicados son exactamente `checksums.txt` (105 b) y `kn-macos-arm64.tar.gz` (2 257 876 b). Ningún artefacto de Linux, Windows ni Intel.
...
- [x] T032 [US4] ✅ `detect_os()` refuses anything that is not Apple Silicon macOS, before any network request, naming the supported platform and exiting non-zero. Original: Narrow platform detection in `install.sh` (lines ~119-170) to Apple Silicon macOS, refusing everything else before any network request, with a message naming the supported platform and a non-zero exit (FR-003, C3.1, C3.2, SC-005)
- [x] T033 [US4] ✅ **Retained and verified.** V10 run under `arch -x86_64 /bin/bash`: `uname=x86_64 proc_translated=1` → probe fired → "Using ARM64 binary" → `Detected OS: macos (arm64)`. Note: `arch -x86_64 bash` fails with "Bad CPU type" because Homebrew's bash is arm64-only — `/bin/bash` (universal) is required to run this test. Original: **Retain the Rosetta probe** at `install.sh:143-149`. It looks like dead code under an arm64-only policy and is not: it detects `x86_64` on macOS, tests whether the process is translated, and rewrites `ARCH` to `arm64`. Deleting it makes the installer refuse the maintainer's own M4 whenever it is invoked from a translated terminal — a regression created by the cleanup itself. Narrow the *genuine Intel* outcome to a refusal; keep the architecture check that distinguishes genuine Intel from translated Apple Silicon (C3.3, research R7, FR-004, spec edge case 6)
- [x] T034 [US4] ✅ Collapsed to a single `asset_name="kn-macos-arm64.tar.gz"`; the OS/arch branching is gone since detection already refused everything else. Original: Reduce artifact selection in `install.sh` (lines ~384-390) to the single `kn-macos-arm64.tar.gz` name, removing the Linux and Intel branches (C3.4)
- [x] T035 [US4] ✅ **Confirmed applied, not forgotten.** Install path and PATH handling untouched. Original: Leave the install destination and PATH handling in `install.sh` unchanged. Only platform detection and artifact selection are in scope (C3.5)
- [x] T036 [P] [US4] ✅ Homebrew is now the first and recommended method, with the exact string `brew install kobogithub/knowledge/kn`. Platform list reduced to one entry; Windows and "Package Managers (Coming Soon)" sections deleted; a callout at the top of Installation states the restriction. Also removed the "Cross-platform support (Windows symlinks)" roadmap item. Original: In `README.md`: reduce "Supported Platforms" (lines ~150-153) to Apple Silicon macOS only; delete the Windows installation section (~205-214); delete the "Package Managers (Coming Soon)" section advertising APT and DNF (~218-226); make Homebrew the primary install, and assert the command string is exactly `brew install kobogithub/knowledge/kn` (FR-005, FR-014, FR-015, FR-016)
- [x] T037 [P] [US4] ✅ Same edits applied, same command string. Original: Apply the same edits to `README_ES.md`: platforms (~126-129), Windows section (~181-190), "Gestores de Paquetes (Próximamente)" (~194-202), Homebrew as primary install with the identical command string (FR-005, FR-014, FR-015, FR-016)
- [x] T038 [US4] ✅ `docs/release/RELEASE.md` rewritten. The manual formula edit, the manual tap `cp` and the APT steps are gone, replaced by the three-job pipeline and an explicit warning not to hand-edit the tap. Original: Rewrite the release procedure in `RELEASE.md`: delete the manual formula-edit steps (~175-193), delete the manual tap `cp` under "Post-Release Tasks" (~283-296), delete the APT repository steps, and delete the `Formula/kn.rb` update items (~76, ~107). Replace with a short statement that the tap is updated automatically by `publish-formula`. This prose *is* the process that failed — it is being deleted, not documented better (research R1, R8)
- [x] T039 [US4] ✅ **SC-006 passes.** The only surviving mentions of Linux/Windows/Intel outside `CHANGELOG.md` and `specs/` are the explicit "not supported" statements and the Rosetta probe's comments. Original: Run the SC-006 grep assertion: search the repository for claims of Linux, Windows or Intel Mac support. Zero results expected outside `CHANGELOG.md` and `specs/`. Changelog entries describing past releases are historical record and stay untouched — the same precedent 002 set in its T020 (research R8)
- [x] T040 [US4] ✅ **V9 and V10 both pass.** V9: in `debian:bookworm`, the script printed "Unsupported OS: linux-gnu / kn supports macOS on Apple Silicon only" and exited 1 **before any download**. V10: succeeded natively and under `arch -x86_64` (see T033). Original: Run quickstart **V9** (install script refuses on Linux via container, downloads nothing, non-zero exit) and **V10** (install script succeeds on Apple Silicon both natively and under `arch -x86_64`, both reporting the same version)
...
- [x] T041 [P] [US5] ✅ Deleted. Original: Delete `kn.spec`. Verified stale during planning: it declares `Version: 0.1.0` while the project ships `0.10.0`, so it has not produced a current package in nine minor versions. Deletion is strictly a cleanup with no loss (FR-018, research R8)
- [x] T042 [P] [US5] ✅ Deleted (7 files). Original: Delete the `debian/` packaging directory (FR-018)
- [x] T043 [P] [US5] ✅ Both deleted — `install.ps1` and `docs/packaging/` (3 files). 12 files removed in total. Original: Delete `install.ps1`, and `docs/packaging/` if present after the T002 rebase — that directory is created by `002` (FR-018)
- [x] T044 [US5] ✅ **SC-007 passes.** `docs/README.md` had a Packaging section linking all three deleted guides — removed. `README.md`'s "Platform-Specific Guides" block removed. No live reference remains; the only hits are `CHANGELOG.md` and `specs/`, both historical record per the precedent 002 set in its T020. Original: Repoint or delete every inbound reference catalogued in research R8: the packaging doc links at `README.md:230-235`, and any surviving mention in `RELEASE.md`. Then run the SC-007 grep assertion — zero results for the deleted files, and no surviving file linking to them (FR-017)
- [x] T045 [US5] ✅ Deletion commit `7b8c35d`; `git show HEAD~1:kn.spec` and `HEAD~1:install.ps1` both return their contents. No history rewritten. Original: Confirm the deleted content remains retrievable from git history and that no history was rewritten: `git log --diff-filter=D --name-only -- kn.spec debian install.ps1` returns the deletion commit (FR-019)
...
- [x] T046 Add a `CHANGELOG.md` entry describing the platform narrowing as a **breaking change** for anyone on Linux, Windows or an Intel Mac, and naming Homebrew as the primary install. Per the project's commit conventions this warrants a `!` breaking-change marker and a MAJOR bump consideration
  - **Ya existía**: `CHANGELOG.md`, sección `## [0.11.0] - 2026-08-17`, encabezada `### ⚠️ BREAKING — kn now supports Apple Silicon macOS only`. Nombra Linux/Windows/Intel, dice que v0.10.0 sigue descargable, y bajo `### Added` pone Homebrew como vía primaria de instalación.
  - **La consideración de MAJOR está resuelta y registrada ahí mismo**: "The version number stays below 1.0.0 deliberately — pre-1.0 convention puts breaking changes in the minor position". Por eso el salto fue 0.10.0 → **0.11.0** (MINOR) y no 1.0.0. La tarea quedó sin tildar, no sin hacer.
- [ ] T047 Run the full quickstart **V1–V11** and record the outcome of each in this file, including any check recorded as unverified **with its reason**. The precedent from `002` is that an unverified check is reported honestly rather than quietly checked off
  - Parcial al 2026-09-12: **V5 en verde** (ver T031) y **V7 en verde** (ver T027). El resto sigue sin correr.
  - Bloqueada por T027 por la misma razón: V6 y V8 dependen de un release nuevo, y el registro V1–V11 no se cierra a medias.
- [x] T048 [P] Confirm `cargo fmt --check`, `cargo clippy -D warnings` and `cargo test` are still green. This feature should not have touched `cli/` at all — a change here means scope leaked
  - **Verificado 2026-09-12 sobre `dev` en `6afc389`** (kn 0.12.1): `cargo fmt --check` limpio, `cargo clippy --all-targets -- -D warnings` sin warnings, `cargo test` 8 passed / 0 failed.
- [x] T049 [P] ~~**Non-blocking follow-up, not this feature's scope**: `.specify/memory/constitution.md` is still an unfilled template~~ — **RESUELTO**. La constitución se ratificó en **v1.0.0** (deuda T040 de `docs/STATUS.md`): cero placeholders `[PRINCIPLE_N_NAME]`, cinco principios derivados de artefactos ya firmados y de ADR aceptados. El Constitution Check de la próxima iniciativa ya no pasa por vacuidad.
